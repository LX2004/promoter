...
```
After initiating the process, the diffusion model training will commence. If device memory is insufficient, adjust the batch size parameter. The trained diffusion model will be stored in the "model" folder that program will automatically create. Next, promoters can be generated by executing the following command. Before running the command, however, update the "model_path" variable in the "train_generate_E_coli_promoter/generate_promoters.py" file to reflect the name of the trained model.
